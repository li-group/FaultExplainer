...
class FaultDetectionModel:
    def __init__(self, n_components=0.9, alpha=0.05):
        ...
        self.post_fault_threshold = 20  # Number of data points to collect after initial fault detection
    ...
    def set_t2_threshold(self):
        ...
        scaling_factor = (self.a*(self.n-1)*(self.n+1))/(self.n*(self.n - self.a))
        self.t2_threshold = scaling_factor*f.ppf(1-self.alpha, self.a, self.n-self.a)
        ...
        if anomaly:
            if self.current_fault_id is None:
                ...
                if self.post_fault_data_count == self.post_fault_threshold:
                    ...
                    for callback in self.fault_callbacks:
                        callback({"data": data_point, "fault_id": self.current_fault_id})
        else:
        ...
        for i, t2_stat in enumerate(self.data_buffer['T^2_stat']):
            if t2_stat > self.t2_threshold and not in_fault:
                ...
                fig.add_shape(type="rect",
                                x0=start_fault, y0=0, x1=end_fault, y1=1,
                                line=dict(width=0),
                                fillcolor="red", opacity=0.2,
                                layer="below", xref="x", yref="paper")
                in_fault = False

        # Check if the last data point is still in a fault period
        if in_fault:
            fig.add_shape(type="rect",
                            x0=start_fault, y0=0, x1=len(self.data_buffer), y1=1,
                            line=dict(width=0),
                            fillcolor="red", opacity=0.2,
                            layer="below", xref="x", yref="paper")
    ...
    def t2_contrib(self, timestamp):
        x = self.data_buffer[self.data_buffer['timestamp']==timestamp][self.feature_names]
    ...
    def plot_t2_contributions(self, start_index, end_index=None):
        ...
        animation_df = pd.DataFrame(contributions, columns=feature_names, index=timestamps).reset_index()
        animation_df = animation_df.melt(id_vars='timestamp', var_name='Feature', value_name='T2 Contribution')

        # Create Animated Bar Plot
        fig = px.bar(animation_df,
                     x='Feature',
                     y='T2 Contribution',
